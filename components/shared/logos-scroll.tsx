...
export default function LogoScroll() {
  ...
  return (
    <div className="flex bg-neutral-100 items-center flex-col max-h-[40rem] h-screen w-screen  justify-center">
      <h1 className="responsive-subtitles mb-20">Trusted by 150+ companies</h1>
      <div className="flex gap-2 xl:gap-10 2xl:gap-16 flex-row">
        {logos.map((logo, index) => (
          <div className="items-center flex-row relative justify-center flex" key={index}>
            <Image
              src={logo}
              className="opacity-800 mx-5 relative h-20 w-20 md:h-20 md:w-20"
              alt={`Logo ${index + 1}`}
            />
          </div>
        ))}
